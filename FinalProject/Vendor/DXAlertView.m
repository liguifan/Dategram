...
{
    self.alertContentLabel.text = @"Search cancelled";
    [SVProgressHUD dismiss];

    double delayInSecondss = 0.5;
...
{        self.alertTitleLabel.text = @"Search cancelled";
    [SVProgressHUD dismiss];
    double delayInSecondss = 0.5;
    dispatch_time_t popTimes = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayInSecondss * NSEC_PER_SEC));
    dispatch_after(popTimes, dispatch_get_main_queue(), ^(void){
...
- (void)show
{
    ...
    [SVProgressHUD showWithStatus:@"Searching match"];
    });
}
